/*
  PCURE: Parallel Data Clustering Algorithm
  == Copyright (C) 2004,2007 P.E.Hadjidoukas, L. Amsaleg
  == Department of Computer Science, University of Ioannina
  == IRISA/INRIA Renness, France
  This file is part of PCURE.
  PCURE is free software; you can redistribute it and/or modify
  it under the terms of the GNU General Public License as published by
  the Free Software Foundation; either version 2 of the License, or
  (at your option) any later version.
  PCURE is distributed in the hope that it will be useful,
  but WITHOUT ANY WARRANTY; without even the implied warranty of
  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
  GNU General Public License for more details.
  You should have received a copy of the GNU General Public License
  along with PCURE; if not, write to the Free Software
  Foundation, Inc., 59 Temple Place, Suite 330, Boston, MA  02111-1307  USA
*/

#define PARALLEL_PRUNING
//#define ADD_RAND_NOISE

#define NESTED_PARALLELISM

/*#define READ_BINARY_FILE*/

#define FIRST_READ_LPAT	0
#include <limits.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
{
#if 0
	return (my_time_t) omp_get_wtime();
#else
	struct timeval tv;
	gettimeofday(&tv, 0);
	return (double) (tv.tv_sec+tv.tv_usec/1000000.0);
...
void myknn(double **xdata, double *q, int npat, int lpat, int knn, int *nn_x, double *nn_d);
void evaluate(struct BTree *item, DATATYPE **pattern, int lpat, int npat, int startpat, int endpat);
...
int main(int argc, char *argv[])
{
  ...
  int i;
	...
	while ((opt = getopt(argc, argv, "n:k:a:r:d:l:p:m:t:s:f:ucxgw:T:i:by")) != -1) {
		switch (opt) {
		case 'a':
	  	printf("alpha...\n");
	  	printf("optarg = %s\n", optarg);
...
#if 0
		case 't':
	  	if (sscanf(optarg, "%d", &threads) != 1 || threads < 1)
				usage();
	  	printf("threads = %d\n", threads);
	  	break;
#endif
...
*/
	...
	if ((rsize == 1)&&(alpha == 1.0)) {
#if 0
		memcpy(item[pair1].rep[0], item[pair1].mean, lpat*sizeof(DATATYPE));
#endif
		return;
	}
