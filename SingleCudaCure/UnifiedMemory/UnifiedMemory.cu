...
const char *path="/SingleCudaCure/UnifiedMemory";
path=ConcenateString(previous_path,path);

change_directory(path);
show_time_results_for_each_case("UnifiedMemory",BT1,npat,lpat,csize);

if(print_clusters)
{
print_results("UnifiedMemory",GPUitem,npat,*nodes,rsize,lpat);
}
