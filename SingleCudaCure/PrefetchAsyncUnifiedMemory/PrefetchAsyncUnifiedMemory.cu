...
printf("-----------------------------------------------------------PREFETCH ASYNC UNIFIED MEMORY-------------------------------------------------------------------------\n\n");
...
Id_Device(&deviceID,1);

gridsize=(npat+blocksize-1)/blocksize;

CudaErrors(cudaMallocManaged(&temp_indices,gridsize*sizeof(int)));
CudaErrors(cudaMallocManaged(&temp_distances,gridsize*sizeof(double)));

CudaErrors(cudaMallocManaged(&gpair1,gridsize*sizeof(int)));
CudaErrors(cudaMallocManaged(&gmin_dist,gridsize*sizeof(double)));

CudaErrors(cudaMallocManaged(&nodes,sizeof(int)));
CudaErrors(cudaMallocManaged(&pruned_nodes,sizeof(int)));
*pruned_nodes=0;

CudaErrors(cudaMallocManaged(&counter,sizeof(int)));
CudaErrors(cudaMallocManaged(&counter1,sizeof(int)));

CudaErrors(cudaMallocManaged(&indices,npat*sizeof(int)));
CudaErrors(cudaMallocManaged(&lpair1,npat*sizeof(int)));
CudaErrors(cudaMallocManaged(&lmin_dist,npat*sizeof(double)));

CudaErrors(cudaMallocManaged(&min_dist,sizeof(double)));

*nodes=npat;

CudaErrors(cudaMemPrefetchAsync(gpair1,gridsize*sizeof(int),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(gmin_dist,gridsize*sizeof(double),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(nodes,sizeof(int),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(pruned_nodes,sizeof(int),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(counter,sizeof(int),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(counter1,sizeof(int),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(indices,npat*sizeof(int),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(lpair1,npat*sizeof(int),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(lmin_dist,npat*sizeof(double),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(gmin_dist,sizeof(double),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(temp_indices,gridsize*sizeof(int),deviceID,0));
CudaErrors(cudaMemPrefetchAsync(temp_distances,gridsize*sizeof(double),deviceID,0));



/*-----------------------------------------------------------------------------GPUitem---------------------------------------------------------------------------------*/

CudaErrors(cudaMallocManaged(&GPUitem,npat*sizeof(BTree)));
CudaErrors(cudaMemPrefetchAsync(GPUitem,npat*sizeof(BTree),deviceID,0));
...
CudaErrors(cudaMallocManaged(&rep,rsize*sizeof(DATATYPE*)));
CudaErrors(cudaMemPrefetchAsync(rep,rsize*sizeof(DATATYPE*),deviceID,0));
...
CudaErrors(cudaMallocManaged(&rep1,lpat*sizeof(DATATYPE)));
CudaErrors(cudaMemPrefetchAsync(rep1,lpat*sizeof(DATATYPE),deviceID,0));
...
CudaErrors(cudaMallocManaged(&mean,lpat*sizeof(DATATYPE)));
CudaErrors(cudaMemPrefetchAsync(mean,lpat*sizeof(DATATYPE),deviceID,0));
...
CudaErrors(cudaMallocManaged(&pats,npat*sizeof(int*)));
CudaErrors(cudaMemPrefetchAsync(pats,npat*sizeof(int*),deviceID,0));

for(unsigned int i=0;i<npat;i++)
{
CudaErrors(cudaMallocManaged(&pats1,nnpc*sizeof(int)));  
CudaErrors(cudaMemPrefetchAsync(pats1,nnpc*sizeof(int),deviceID,0)); 
...
/*-----------------------------------------------------------------------GPU_merge_tmp_item-----------------------------------------------------------------------*/
CudaErrors(cudaMallocManaged(&GPU_merge_tmp_item,sizeof(BTree)));
CudaErrors(cudaMemPrefetchAsync(GPU_merge_tmp_item,sizeof(BTree),deviceID,0));

DATATYPE **rep2,*rep3;
CudaErrors(cudaMallocManaged(&rep2,(2*rsize)*sizeof(DATATYPE*)));
CudaErrors(cudaMemPrefetchAsync(rep2,(2*rsize)*sizeof(DATATYPE*),deviceID,0));

for(unsigned int i=0;i<(2*rsize);i++)
{
CudaErrors(cudaMallocManaged(&rep3,lpat*sizeof(DATATYPE)));
CudaErrors(cudaMemPrefetchAsync(rep3,lpat*sizeof(DATATYPE),deviceID,0));  
...
/*-----------------------------------------------------------------------KERNEL_GPUnnb-----------------------------------------------------------------------------------*/
CudaErrors(cudaMallocManaged(&KERNEL_GPUnnb,npat*sizeof(nnb_info)));
CudaErrors(cudaMemPrefetchAsync(KERNEL_GPUnnb,sizeof(nnb_info),deviceID,0));
...
const char *path="/SingleCudaCure/PrefetchAsyncUnifiedMemory";
path=ConcenateString(previous_path,path);

change_directory(path);
show_time_results_for_each_case("PrefetchAsyncUnifiedMemory",BT1,npat,lpat,csize);

if(print_clusters)
{
print_results("PrefetchAsyncUnifiedMemory",GPUitem,npat,*nodes,rsize,lpat);
}
...
CudaErrors(cudaFree(pruned_nodes));
CudaErrors(cudaFree(nodes));
CudaErrors(cudaFree(lpair1));
CudaErrors(cudaFree(lmin_dist));
CudaErrors(cudaFree(counter));
CudaErrors(cudaFree(counter1));
