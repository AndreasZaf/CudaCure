...
CudaErrors(cudaHostAlloc((void**)&gpair1,gridsize*sizeof(int),cudaHostAllocDefault));
CudaErrors(cudaHostAlloc((void**)&gmin_dist,gridsize*sizeof(double),cudaHostAllocDefault));

CudaErrors(cudaHostAlloc((void**)&temp_indices,gridsize*sizeof(int),cudaHostAllocDefault));
CudaErrors(cudaHostAlloc((void**)&temp_distances,gridsize*sizeof(double),cudaHostAllocDefault));

CudaErrors(cudaHostAlloc((void**)&nodes,sizeof(int),cudaHostAllocDefault));
CudaErrors(cudaHostAlloc((void**)&pruned_nodes,sizeof(int),cudaHostAllocDefault));
*pruned_nodes=0;
*nodes=npat;

CudaErrors(cudaHostAlloc((void**)&counter,sizeof(int),cudaHostAllocDefault));
CudaErrors(cudaHostAlloc((void**)&counter1,sizeof(int),cudaHostAllocDefault));

CudaErrors(cudaHostAlloc((void**)&indices,npat*sizeof(int),cudaHostAllocDefault));
CudaErrors(cudaHostAlloc((void**)&lpair1,npat*sizeof(int),cudaHostAllocDefault));
CudaErrors(cudaHostAlloc((void**)&lmin_dist,npat*sizeof(double),cudaHostAllocDefault));

CudaErrors(cudaHostAlloc((void**)&min_dist,sizeof(double),cudaHostAllocDefault));

/*-----------------------------------------------------------------------------GPUitem---------------------------------------------------------------------------------*/

CudaErrors(cudaHostAlloc((void**)&GPUitem,npat*sizeof(BTree),cudaHostAllocDefault));
...
CudaErrors(cudaHostAlloc((void**)&rep,rsize*sizeof(DATATYPE*),cudaHostAllocDefault));
...
CudaErrors(cudaHostAlloc((void**)&rep1,lpat*sizeof(DATATYPE),cudaHostAllocDefault));
...
CudaErrors(cudaHostAlloc((void**)&mean,lpat*sizeof(DATATYPE),cudaHostAllocDefault));
...
CudaErrors(cudaHostAlloc((void**)&pats,npat*sizeof(int*),cudaHostAllocDefault));
for(unsigned int i=0;i<npat;i++)
{
CudaErrors(cudaHostAlloc((void**)&pats1,nnpc*sizeof(int),cudaHostAllocDefault));   
...
CudaErrors(cudaHostAlloc((void**)&GPU_merge_tmp_item,sizeof(BTree),cudaHostAllocDefault));

DATATYPE **rep2;
DATATYPE *rep3;
CudaErrors(cudaHostAlloc((void**)&rep2,(2*rsize)*sizeof(DATATYPE*),cudaHostAllocDefault));

for(unsigned int i =0; i<(2*rsize); i++)
{
CudaErrors(cudaHostAlloc((void**)&rep3,lpat*sizeof(DATATYPE),cudaHostAllocDefault));  
...
/*-----------------------------------------------------------------------KERNEL_GPUnnb-----------------------------------------------------------------------------------*/
CudaErrors(cudaHostAlloc((void**)&KERNEL_GPUnnb,npat*sizeof(nnb_info),cudaHostAllocDefault));
...
const char *path="/SingleCudaCure/PinnedMemoryCHA";
path=ConcenateString(previous_path,path);

change_directory(path);
show_time_results_for_each_case("PinnedMemoryCHA",BT1,npat,lpat,csize);

if(print_clusters)
{
print_results("PinnedMemoryCHA",GPUitem,npat,*nodes,rsize,lpat);
}
